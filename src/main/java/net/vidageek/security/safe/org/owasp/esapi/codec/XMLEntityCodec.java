 ...
 * 
 * It's also worth noting that unlike the HTMLEntityCodec, a trailing semicolon
 * is required and all valid codepoints are accepted.
 * 
 * Note that it is a REALLY bad idea to use this for decoding as an XML document
 * can declare arbitrary entities that this Codec has no way of knowing about.
 * Decoding is included for completeness but it's use is not recommended. Use a
 * XML parser instead!
 */
public class XMLEntityCodec extends Codec {
	 ...
	 * 
	 * @param immune
	 *            characters that should not be encoded as entities
	 */
	@Override
	 ...
	 * 
	 * @param input
	 *            The input to read from. It is assumed that input is positioned
	 ...
	 * @param i
	 *            the integer
	 * @return i as a Character or null if i is a invalid code point or outside
	 *         of the Java char range.
	 */
	 ...
	 * @param input
	 *            The input to read from. It is assumed that input is positioned
	 *            at the character after the &amp;# and that the next char is
	 *            not a 'x' or 'X'.
	...
	/**
	 * 
	 * Converts the rest of a named entity to a character. null if no decoding
	 * is possible.
	 * 
	 * @param input
	 *            The input to read from. It is assumed that input is positioned
